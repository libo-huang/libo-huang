# Libo Huang / 黄礼泊

I'm an assistant research fellow at a national research institute in China (PhD 2021). I do research, supervise students, and publish papers.

国内某科研院所的助理研究员，21年人工智能博士毕业。平时做研究、带学生、发论文。

---

## I help small teams with technical problems

If you're stuck on model accuracy, weird data, slow code, or a tricky design, I might be able to help.

你的技术团队遇到了模型精度、数据异常、代码性能或方案设计上的难题？我可以帮你看看。

---

## Why me? / 为什么找我？

- Ph.D. + research experience.
- I teach, so I explain things clearly.
- I publish, so I think rigorously.
- Institute affiliation is a public record.

博士学历 + 科研经验。带学生，把复杂问题讲清楚。发论文，思考问题严谨。隶属机构公开可查。

---

## How we work / 怎么合作？

**1. Free 30-min chat (no obligation):** You describe your problem, and I give initial thoughts.  
**2. If it makes sense:** We agree on a small project or hourly support (like sponsored research).  
**3. You get a solution:** Code, report, or a clear diagnosis.

**1、免费聊半小时：** 你讲问题，我给初步想法。  
**2、如果值得继续：** 按小项目或按小时协作（类似横向课题）。  
**3、给你解决方案：** 代码、报告、或一个清晰的诊断。

> I don't list prices here because every case is different. For a rough idea: typical engagements range from a few hundred to a couple thousand USD. We'll talk after I understand your needs.

---

## Contact / 联系

- **Book a free chat** → [Calendly link](https://calendly.com/libohuang)
- **Email** → www.huanglibo@gmail.com 
- **WeChat** → libo_huang (请备注“技术咨询”)

I reply within 24h.

---

## My GitHub (just for fun)

<picture>  
  <img alt="github contribution snake" src="https://raw.githubusercontent.com/libo-huang/libo-huang/output/github-contribution-grid-snake.svg">
</picture>
